...
from keras.models import Sequential
from keras.layers import Dense, Dropout, Activation, SimpleRNN, LSTM, TimeDistributed
...
model1 = Sequential()
model1.add(LSTM(128, activation='tanh',return_sequences=True, recurrent_dropout=0.28, input_shape=(None, 39)))
...
out= model1.fit(train_in, train_out, epochs=91)

test_out_pred = model1.predict_classes(test_in)
train_out_pred=model1.predict_classes(train_in)


train_ind_mov=train_out_pred
test_ind_mov=test_out_pred
...
x=SimpleRNN(100, activation='tanh',recurrent_dropout=0.2, dropout=0.1, return_sequences=True )(main_input, training=True)
...
for sim in range(0,100, 1):
  ...
  filename='/storage/work/d/dzw5320/sims/LSTM/'+str(sim)+'LSTMind.csv'
