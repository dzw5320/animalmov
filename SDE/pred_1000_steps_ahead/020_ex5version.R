...
get_pred <- function(parallel_iter) {
  ...
  for (t_c in c((11515 + 1):(11515 + n_sim))) { #n_sim time points starting at time 11516
    cat("\n", "t: ", t_c - 11515, "\n")
      ...
      if (new_row$movt2 == "yes") {
        ...
        while (done == FALSE) {
          ...
          while(inpoly == FALSE) {
            ...
            inpoly <- ifelse(pracma::inpolygon(x = new_row$x,
                                               y = new_row$y,
                                               xp = P[,1],
                                               yp = P[,2],
                                               boundary = TRUE),
