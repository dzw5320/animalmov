...
x=Dense(350, activation='relu')(main_input)
x = Dropout(0.2)(x)
...
out=model1.fit([train_in], [train_out_hot[:,0,:], train_out_hot[:,1,:],  train_out_hot[:,2,:],  train_out_hot[:,3,:],  train_out_hot[:,4,:],  train_out_hot[:,5,:],  train_out_hot[:,6,:],  train_out_hot[:,7,:],  train_out_hot[:,8,:],  train_out_hot[:,9,:],
                      ...
                      train_out_hot[:,70,:], train_out_hot[:,71,:],  train_out_hot[:,72,:]], verbose=0,epochs=100)
...
main_input=Input(shape=(1000,), name='main_input',dtype='float32')
x = Dropout(0.2)(main_input, training=True)
...
#Calculate MSPE for training set
...
for sim in np.arange(0, 100, 1):
  ...
  for t in range(1000):
    ...
    for i in np.arange(73):
      pred.iloc[i, 36]=getClass(predmov[i]).copy()
    predvel=model.predict(getdatastd)
    predvel=scaler1.inverse_transform(predvel).copy()
    for i in range(73):
      if(pred.iloc[i,36]==2):
  ...
  filename='/storage/home/d/dzw5320/MachineLearning/Results3/NN/keras/sims/'+str(sim)+'NNcol.csv'
