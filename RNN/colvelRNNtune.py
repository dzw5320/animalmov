...
L3=np.array([0, 150,200 ])
LR=np.array([ 0.001, 0.0005 ])
eps=np.array([50, 100 ])



drop_Series=np.repeat(drop, 72)
recc_drop_Series=np.tile(np.repeat(recc_drop, 36), 2)
L1_Series=np.tile(np.repeat(L1, 36), 4)
L2_Series=np.tile(np.repeat(L2, 12), 4)
L3_Series=np.tile(np.repeat(L3, 4), 12)
LR_Series=np.tile(np.repeat(LR,2), 36)
eps_Series=np.tile(eps, 72)
MSPEtrain=np.zeros(144)
MSPEtest=np.zeros(144)

tune=np.column_stack((drop_Series,recc_drop_Series,L1_Series,L2_Series,L3_Series,LR_Series, eps_Series, MSPEtrain,  MSPEtest  ))

for r in range(0,144,1):
  ...
  x=SimpleRNN(350, activation='tanh', recurrent_dropout=tune[r,1], dropout=tune[r,0], return_sequences=True )(main_input, training=True)
  ...
  tune[r, 7]=check.sum(axis=0).sum()/(11511*73)
  ...
  tune[r, 8]=check.sum(axis=0).sum()/(2883*73)
  np.save('RNNcolveltune.npy', tune)
