...
from keras.layers import Input, Embedding, Dense, Dropout, LSTM, TimeDistributed, Reshape, Flatten, SimpleRNN
...
model1 = Sequential()
model1.add(SimpleRNN(64, activation='tanh',return_sequences=True,dropout=0.12, recurrent_dropout=0.08, input_shape=(None, 39)))
model1.add(Dense(32, activation='tanh'))
model1.add(Dropout(0.36))
model1.add(Dense(3, activation='softmax'))
model1.compile(loss='categorical_crossentropy',metrics=['accuracy'],
              optimizer=Nadam(lr=lr_normalizer(6.04, Nadam)))
out= model1.fit(train_in, train_out, epochs=82, verbose=0)


test_ind_mov= model1.predict_classes(test_in)
train_ind_mov=model1.predict_classes(train_in)

train_ind_mov=np.reshape(train_ind_mov, (11511*73, 1))
test_ind_mov=np.reshape(test_ind_mov, (2883*73, 1))

#np.save('train_RNN_indmov.npy', train_out_pred)
#np.save('test_RNN_indmov.npy', test_out_pred)

#model1_json = model1.to_json()
#with open("/storage/home/d/dzw5320/MachineLearning/Results3/RNN/keras/RNNindmov.json", "w") as json_file:
#    json_file.write(model1_json)
#model1.save_weights("/storage/home/d/dzw5320/MachineLearning/Results3/RNN/keras/RNNindmov.h5")
...
x=SimpleRNN(75, activation='tanh',recurrent_dropout=0.1, dropout=0.1, return_sequences=True )(main_input, training=True)
...
#with open("/storage/home/d/dzw5320/MachineLearning/Results3/RNN/keras/RNNindvel.json", "w") as json_file:
#    json_file.write(model2_json)
#model2.save_weights("/storage/home/d/dzw5320/MachineLearning/Results3/RNN/keras/RNNindvel.h5")
...
#np.save('train_RNN_indvel.npy', pred_train)
#np.save('test_RNN_indvel.npy', pred_test)
...
for sim in range(0,100, 1):
  ...
  for t in range(1000):
    ...
    getdata=pred_.iloc[:, np.r_[9:36,37:pred_.shape[1]]].values
    getdatastdmov=scaler1.transform(getdata)
    getdatastdreg=scaler2.transform(getdata)
    getdatastdmov=np.reshape(getdatastdmov, ( 73,-1, 39))
    #getdatastdmov=np.transpose(getdatastdmov, (1, 0, 2))
    getdatastdreg=np.reshape(getdatastdreg, ( 73,-1, 39))
    #getdatastdreg=np.transpose(getdatastdreg, (1, 0, 2))
    predmov=model1.predict_proba(getdatastdmov)
    #pred['movt']=predmov[:, -1]
    #if(t==0):
    #  predseries['movt']=pred['movt'].copy()
  
    for i in range(73):
      pred.iloc[i,36]=getClass(predmov[i, -1, :])
      if(pred.iloc[i,36]==2):
        getdatastdregind=np.reshape(getdatastdreg[i, :, :], ( 1,-1, 39))
        vxpred=model2.predict(getdatastdregind)
        pred.iloc[i,5:7 ]=scaler3.inverse_transform(vxpred[0, -1, :])
  ...
  filename='/storage/home/d/dzw5320/MachineLearning/Results3/RNN/keras/sims/'+str(sim)+'RNNind.csv'
